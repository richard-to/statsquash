...
class Extractor:
    ...
    def extract(self, location, today, interval):
        data = []
        if os.path.isfile(location):
            path, filename = os.path.split(location)
            label, ext = os.path.splitext(filename)
            result = (subprocess.check_output([location, str(today), interval])).strip()
            ...
            for dirname, dirnames, filenames in os.walk(location):
                for filename in filenames:
                    ...
                    label, ext = os.path.splitext(filename)
                    result = (subprocess.check_output([script, str(today), interval])).strip()
                    data.append((label, result, today))
        return data
...
class Loader:
    ...
    def __init__(self, db_params, table):
        ...
        self.table = table
        self.base_query = "insert into %s (label, value, created_at) values " % (self.table)
